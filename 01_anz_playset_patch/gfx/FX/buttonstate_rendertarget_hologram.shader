...
PixelShader =
{
	...
	[[
		float4 main( VS_OUTPUT v ) : PDX_COLOR
		{
			...
			const float4 vEdgeColorBoost = float4(0.0, 0.0, 0.0, 0.0);// anz_alter
			...
			const float3 ShadowColor = float3(0.0, 0.0, 0.0); // anz_alter
			const float3 HighlightColor = float3(0.0, 0.0, 0.0); // anz_alter
			float4 Fill;
